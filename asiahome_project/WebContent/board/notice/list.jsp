	...
	<tr>
		<td colspan="5" style="text-align:center">
			<c:if test="${beginPage>10 }">
				<a href="<%=request.getContextPath()%>/NoticeBoard.do?command=notice_board_list?${beginPage-1}">이전</a>
			</c:if>
			<c:forEach var="pageNum" begin="${beginPage }" end="${endPage }">
				<c:choose>
					...
					<c:otherwise>
						<a href="<%=request.getContextPath()%>/NoticeBoard.do?command=notice_board_list?${pageNum}">[${pageNum}]</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${endPage<totalPageCount }">
				<a href="<%=request.getContextPath()%>/NoticeBoard.do?command=notice_board_list?${endPage+1}">다음</a>
			</c:if>
		</td>
	</tr>
	<tr>		
		<td colspan="5" style="text-align:center">
			...
			<input type="text" name="search_bar"><input type="submit" value="검색">
		</td>
	</tr>
</table>
</form>